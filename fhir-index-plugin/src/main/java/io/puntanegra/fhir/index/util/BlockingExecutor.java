 ...
 * the event of all tasks being run to conclusion. Client code which actively
 ...
 * the queue is full, but the BlockingExecutor will block once the queue is
 * full.
 *
...
public class BlockingExecutor extends ThreadPoolExecutor {
	 ...
	 * BlockThenRunPolicy that is defined in this class. This constructor allows
	 * to give a timeout for the wait on new task insertion and to react upon
	 ...
	 *            given time
	 * @param queueSize
	 *            is the size of the queue. This number should be at least as
	 *            the pool size to make sense (otherwise there are unused
	 *            threads), thus if the number sent is smaller, the poolSize is
	 *            used for the size of the queue. Recommended value is twice the
	 *            poolSize.
	 * @param keepAliveTime
	 ...
	 *            calling the BlockingTimeout callback
	 ...
	 *            Callable is Boolean, indicating whether to keep blocking
	 *            (true) or stop (false). In case false is returned from the
	 *            blockingTimeCallback, this executer will throw a
	 *            RejectedExecutionException
	 */
	public BlockingExecutor(int poolSize, int queueSize, long keepAliveTime, TimeUnit keepAliveTimeUnit,
	...
	public void execute(Runnable task) {
		...
		try {
			super.execute(task);
		} catch (RuntimeException | Error e) { // specifically handle
												// RejectedExecutionException
	...
	/**
	 * Internally calls on super's setCorePoolSize and setMaximumPoolSize
	 ...
	 * task is currently sent to the ThreadPool not from this thread context.
	 * Thus it is not safe to call this method in case there are several threads
	 * feeding the TreadPool with tasks (calling execute). The safe way to call
	 * this method is from the thread that is calling execute and when there is
	 * only one such thread. Note that this method differs from awaitTemination,
	 * as it can be called without shutting down the ThreadPoolExecuter.
	 *
	...
	/**
	 * A blocking wait for this ThreadPool to be in idle state or a certain
	 * timeout to elapse. Works the same as the await() method, except for
	 * adding the timeout condition.
	 *
	 * @param timeout
	 *            The timeout.
	 * @param timeUnit
	 *            The time unit.
	 * @return false if the timeout elapsed, true if the synch event we are
	 ...
	 * ThreadPoolExecutor has finished running all the tasks given to its
	 * execute method.
	 */
	private class Synchronizer {
		...
		/**
		 * Inner implementation for supporting the
		 * BlockingExecutor.await(timeout, timeUnit).
		 *
		 * @param timeout
		 *            The await time.
		 * @param timeUnit
		 *            The await {@link TimeUnit}.
		 * @throws InterruptedException
		 *             when the internal condition throws it.
	...
	/**
	 * This Policy class enforces the blocking feature of the BlockingExecutor.
	 * It does so by invoking the BlockingQueue's put method (instead of the
	 * offer method that is used by the standard implementation of the
	...
	private static class BlockThenRunPolicy implements RejectedExecutionHandler {
		...
		/**
		 * When this method is invoked by the ThreadPoolExecutor's reject method
		 * it simply asks for the Executor's Queue and calls on its put method
		 * which will Block (at least for the ArrayBlockingQueue).
		 *
